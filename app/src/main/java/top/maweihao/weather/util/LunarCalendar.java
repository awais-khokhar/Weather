...
public class LunarCalendar {
    ...
    private final static String[] solarHoliday = new String[]{
            ...
            "1226 毛泽东诞辰纪念"};
    //24节气
    private final static String[] sTermInfo = new String[]{
            // 时节     气候
            "小寒", "大寒",
            "立春", "雨水",
            "惊蛰", "春分",
    ...
    private final static String[] constellations = new String[]{
    ...
    private final static String[] yi_string = new String[]{
    ...
    private final static String[] ji_string = new String[]{
    ...
    private final static String[][] jcName = new String[][]{
    ...
    private final static String[] Gan = new String[]{"甲", "乙", "丙", "丁", "戊", "己", "庚",
            "辛", "壬", "癸"};
    private final static String[] Zhi = new String[]{"子", "丑", "寅", "卯", "辰", "巳", "午",
            "未", "申", "酉", "戌", "亥"};
    ...
    private String cyclical() {
        int num = 2017 - 1900 + 36;
        //立春日期
        int term2 = sTerm(2017, 2);
        return (cyclicalm(num));
    }

    private static String getChinaDayString(int day) {//将农历day日格式化成农历表示的字符串
     ...
     * */
    private int getDaysOfTwoDate(int bD, int nY, int nM, int nD) {
        Date baseDate = null;
        Date nowaday = null;
        try {
            baseDate = chineseDateFormat.parse(1900 + "年" + 1 + "月" + bD + "日");
     ...
     * 从农历日期转换成公历日期
     * */
    private Calendar getSunDate(int lunYear, int lunMonth, int lunDay) {
    ...
    String getLunarString(int year, int month, int day) {

        int var = getLunarDateINT(year, month, day);
        int lYear = var / 10000;
        int lMonth = var % 10000 / 100;
        int lDay = var - lYear * 10000 - lMonth * 100;
        String lunY = cyclical() + "年";
        String lunM = "";
        int testMonth = getSunDate(lYear, lMonth, lDay).get(Calendar.MONTH) + 1;
        if (testMonth != 6) {
            lunM = "闰";
        }
        ...
        int animalsYear = 0;
        int term2 = sTerm(2017, 2);//立春
        animalsYear = 2017;
        String Festival = getFestival(2017, 6, 3);
        //return lunY+"-"+lunM+"-"+lunD+"-"+animalsYear(animalsYear);
        return lunM + lunD + " " + Festival;
    }
     ...
     * 返回格式为20140506（int）
     * */
    private int getLunarDateINT(int year, int month, int day) {
        int iYear, LYear, LMonth, LDay, daysOfYear = 0;
        // 求出和1900年1月31日相差的天数
        //year =1908;
        //month = 3;
        //day =3;
        int offset = getDaysOfTwoDate(31, year, month, day);
    ...
    private String getFestival(int year, int month, int day) {//获取公历对应的节假日或二十四节气
        ...
        //公历节假日
        for (i = 0; i < solarHoliday.length; i++) {
    ...
    public String toString() {
        if (Objects.equals(chineseNumber[(month - 1) % 12], "正") && Objects.equals(getChinaDayString(day), "初一"))
            return "农历" + year + "年";
        else if (Objects.equals(getChinaDayString(day), "初一"))
            return chineseNumber[(month - 1) % 12] + "月";
        else
            return getChinaDayString(day);
    ...
    private void setLunarMonth(String lunarMonth) {
    ...
    private int getDateOfSolarTerms(int year, int month) {
    ...
    private String jcrt(String d) {
        ...
        //String yj0 = "",yj1 = "",br = "-";
        if (Objects.equals(d, "建")) jcrjxt = yj0 + yi_string[0] + br + yj1 + ji_string[0];
        if (Objects.equals(d, "除")) jcrjxt = yj0 + yi_string[1] + br + yj1 + ji_string[1];
        if (Objects.equals(d, "满")) jcrjxt = yj0 + yi_string[2] + br + yj1 + ji_string[2];
        if (Objects.equals(d, "平")) jcrjxt = yj0 + yi_string[3] + br + yj1 + ji_string[3];
        if (Objects.equals(d, "定")) jcrjxt = yj0 + yi_string[4] + br + yj1 + ji_string[4];
        if (Objects.equals(d, "执")) jcrjxt = yj0 + yi_string[5] + br + yj1 + ji_string[5];
        if (Objects.equals(d, "破")) jcrjxt = yj0 + yi_string[6] + br + yj1 + ji_string[6];
        if (Objects.equals(d, "危")) jcrjxt = yj0 + yi_string[7] + br + yj1 + ji_string[7];
        if (Objects.equals(d, "成")) jcrjxt = yj0 + yi_string[8] + br + yj1 + ji_string[8];
        if (Objects.equals(d, "收")) jcrjxt = yj0 + yi_string[9] + br + yj1 + ji_string[9];
        if (Objects.equals(d, "开")) jcrjxt = yj0 + yi_string[10] + br + yj1 + ji_string[10];
        if (Objects.equals(d, "闭")) jcrjxt = yj0 + yi_string[11] + br + yj1 + ji_string[11];
    ...
    public ArrayList<String> getyiji(int year, int month) {
        ...
        int dayCyclical = (int) (getDaysOfTwoDate(1, year, month, 1) + 10);
    ...
    public String getLunarDateFromTimeMills(long mill) {
        ...
        return getLunarString(year, month, day);
    }

    public String getLunarDateFromTimeMills () {
        return getLunarDateFromTimeMills(System.currentTimeMillis());
    }
}
